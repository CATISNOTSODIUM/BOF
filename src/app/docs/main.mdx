...
```
x $= ++++.> |  // add 4 to the current block, 
// output its value and move to its consecutive block.
x x x // output 4, 4, 4 memory [ 4 | 4 | 4 | 0 ]
```

You can do bunch of cool things with macros.
```
mov $= [->+<]> | // allocate value from current block
// to the next one.
...
```
cpy $= x=.;>x| // copy the value from this block 
// and transfer to another block.
